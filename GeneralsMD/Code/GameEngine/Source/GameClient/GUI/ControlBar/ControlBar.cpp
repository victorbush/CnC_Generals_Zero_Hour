...
//	TheInGameUI->deselectAllDrawables();
	...
	for( i = 0; i < MAX_PURCHASE_SCIENCE_RANK_1; i++ )
	{
		...
		if( commandButton == NULL || BitTestWW( commandButton->getOptions(), SCRIPT_ONLY ) )
		{
...
Bool CommandButton::isValidToUseOn(const Object *sourceObj, const Object *targetObj, const Coord3D *targetLocation, CommandSourceType commandSource) const
{
	...
	if( BitTestWW( m_options, COMMAND_OPTION_NEED_OBJECT_TARGET ) && !targetObj ) 
	{
		return false;
	}
	...
	if( BitTestWW( m_options, NEED_TARGET_POS ) && !targetLocation ) 
	{
		if( targetObj )
		{
...
{
	return BitTestWW( m_options, CONTEXTMODE_COMMAND );
}
...
void ControlBar::setControlCommand( GameWindow *button, const CommandButton *commandButton )
{
	...
	//
	if( BitTestWW( commandButton->getOptions(), CHECK_LIKE ))
...
void ControlBar::triggerRadarAttackGlow( void )
{
	...
	if(BitTestWW(m_radarAttackGlowWindow->winGetStatus(),WIN_STATUS_ENABLED) == TRUE)
...
void ControlBar::updateRadarAttackGlow ( void )
{
	...
	{
		m_radarAttackGlowWindow->winEnable(!BitTestWW(m_radarAttackGlowWindow->winGetStatus(),WIN_STATUS_ENABLED));
	}
...
void ControlBar::populateSpecialPowerShortcut( Player *player)
{
	...
	for( i = 0; i < m_currentlyUsedSpecialPowersButtons; i++ )
	{
		...
		else
		{
			...
			if( BitTestWW( commandButton->getOptions(), NEED_UPGRADE ) )
			{
			...
			//
			if( BitTestWW( commandButton->getOptions(), NEED_SPECIAL_POWER_SCIENCE ) )
			{
