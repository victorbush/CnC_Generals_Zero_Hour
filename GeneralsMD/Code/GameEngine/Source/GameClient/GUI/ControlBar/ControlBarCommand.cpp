...
void ControlBar::populateCommand( Object *obj )
{
	...
	for( i = 0; i < MAX_COMMANDS_PER_SET; i++ )
	{
		...
		else
		{
			...
			if( BitTestWW( commandButton->getOptions(), SCRIPT_ONLY ) )
			{
				m_commandWindows[ i ]->winHide( TRUE );
				continue;
			}
...
//		else
		...
		//
		if( BitTestWW( command->getOptions(), CHECK_LIKE ))
		{

			// sanity, check like commands should have windows that are check like as well
			DEBUG_ASSERTCRASH( BitTestWW( win->winGetStatus(), WIN_STATUS_CHECK_LIKE ),	
																												...
																												Bool forceDisabledEvaluation ) const
{
	...
	if( BitTestWW( command->getOptions(), MUST_BE_STOPPED ) )
	{
	...
	if (disabled
			&& BitTestWW(command->getOptions(), IGNORES_UNDERPOWERED) 
	...
	if( BitTestWW( command->getOptions(), NEED_UPGRADE ) )
	{
	...
	if( pu && pu->firstProduction() && BitTestWW( command->getOptions(), NOT_QUEUEABLE ) )
	{
...
/// and re-target the firewall without having to stop or move in-betwen.. Thanks for reading 
    ...
    //since the container can be subdued, the above is no longer true, M Lorenzen 8/11
      ...
      if ( obj->isDisabledByType( DISABLED_SUBDUED ) )
		...
		{
			if( !BitTestWW( command->getOptions(), OPTION_ONE ) )
			{
				return COMMAND_AVAILABLE;
			}
