...
//			m_buildToolTipLayout = NULL;
	...
	if(!cmdButton)
		return;
	if(BitTestWW(cmdButton->winGetStyle(), GWS_PUSH_BUTTON))
	{
	...
	else
	{
		...
		if(!BitTestWW(cmdButton->winGetStyle(), GWS_USER_WINDOW) && !BitTestWW(cmdButton->winGetStyle(), GWS_STATIC_TEXT))
			return;
...
void ControlBar::repopulateBuildTooltipLayout( void )
{
	if(!prevWindow || !m_buildToolTipLayout)
		return;
	if(!BitTestWW(prevWindow->winGetStyle(), GWS_PUSH_BUTTON))
		return;
