...
Bool GameWindowManager::isEnabled( GameWindow *win )
{
	...
	if( BitTestWW( win->m_status, WIN_STATUS_ENABLED ) == FALSE )
	{
		return FALSE;
	}
...
Bool GameWindowManager::isHidden( GameWindow *win )
{
	...
	if( BitTestWW( win->m_status, WIN_STATUS_HIDDEN ))
	{
		return TRUE;
	}
																					...
																					WindowMsgData mData2 )
{
	...
	if( msg != GWM_DESTROY && BitTestWW( window->m_status, WIN_STATUS_DESTROYED ) )
		return MSG_IGNORED;
...
Int GameWindowManager::winSetFocus( GameWindow *window )
{
	...
	if( window )
		if( BitTestWW( window->winGetStatus(), WIN_STATUS_NO_FOCUS) )
			return 0;
		...
		if( m_grabWindow )
		{
			...
			switch( msg )
			{
				...
				case GWM_LEFT_DRAG:
				{
					...
					if( BitTestWW( m_grabWindow->m_status, WIN_STATUS_DRAGABLE ) )
					{
		...
		else
		{
			...
			else
			{
				...
				for( window = m_windowList; window; window = window->m_next )
				{
					...
					if( BitTestWW( window->m_status, WIN_STATUS_ABOVE ) &&
							!BitTestWW( window->m_status, WIN_STATUS_HIDDEN ) &&
							...
							mousePos->y <= window->m_region.hi.y)
					{
						...
						if( BitTestWW( window->m_status, WIN_STATUS_ENABLED ) )
						{
				...
				if( window == NULL )
				{
					...
					for( window = m_windowList; window; window = window->m_next )
					{
						...
						if( BitTestWW( window->m_status, WIN_STATUS_BELOW ) &&
								!BitTestWW( window->m_status, WIN_STATUS_HIDDEN ) &&
								mousePos->x >= window->m_region.lo.x &&
								mousePos->x <= window->m_region.hi.x &&
								mousePos->y >= window->m_region.lo.y &&
			...
			if( window )
				if( BitTestWW( window->m_status, WIN_STATUS_NO_INPUT ) )
				{
					if(window->winGetParent() && BitTestWW( window->winGetParent()->winGetInstanceData()->getStyle(), GWS_COMBO_BOX ))
						window = window->winGetParent();
					else
						window = NULL;
				}
...
//							WinActivate( tempWin );
			...
			if( tooltipsOn )
			{
//				if( toolTipWindow && toolTipWindow->winGetParent() && BitTestWW( toolTipWindow->winGetParent()->winGetInstanceData()->getStyle(), GWS_COMBO_BOX ))
...
Int GameWindowManager::drawWindow( GameWindow *window )
{
	...
	if( BitTestWW( window->m_status, WIN_STATUS_HIDDEN ) == FALSE )
	{

		if( !BitTestWW( window->m_status, WIN_STATUS_SEE_THRU ) && window->m_draw )
		...
		if( BitTestWW( window->winGetStyle(), GWS_SCROLL_LISTBOX ) )
			if( BitTestWW( window->m_status, WIN_STATUS_BORDER ) == TRUE &&
					!BitTestWW( window->m_status, WIN_STATUS_SEE_THRU ) )
																										 ...
																										 Bool defaultVisual )
{
	...
	if( BitTestWW( instData->getStyle(), GWS_PUSH_BUTTON ) == FALSE )
	{
	...
	//
	if( BitTestWW( button->winGetStatus(), WIN_STATUS_IMAGE ) )
...

{
	...
	// we MUST have a push button style window to do this
	if( BitTestWW( instData->getStyle(), GWS_CHECK_BOX ) == FALSE )
	{
	...
	// on the device level of the engine
	//
	if( BitTestWW( checkbox->winGetStatus(), WIN_STATUS_IMAGE ) )
...

{
	...
	// we MUST have a push button style window to do this
	if( BitTestWW( instData->getStyle(), GWS_RADIO_BUTTON ) == FALSE )
	{
	...
	// on the device level of the engine
	//
	if( BitTestWW( radioButton->winGetStatus(), WIN_STATUS_IMAGE ) )
...

{
	...
	if( BitTestWW( instData->getStyle(), GWS_TAB_CONTROL ) == FALSE )
	{
	...
	// on the device level of the engine
	//
	if( BitTestWW( tabControl->winGetStatus(), WIN_STATUS_IMAGE ) )
...

{
  ...
  ListboxData *listboxData;
	...
	// we MUST have a push button style window to do this
	if( BitTestWW( instData->getStyle(), GWS_SCROLL_LISTBOX ) == FALSE )
	{
  ...
  listbox->winSetUserData( listboxData );
	...
	if( BitTestWW( listbox->winGetStatus(), WIN_STATUS_IMAGE ))
...

{
	...
	else if ( BitTestWW( instData->getStyle(), GWS_VERT_SLIDER ) ) 
	{
		...
		if( BitTestWW( slider->winGetStatus(), WIN_STATUS_IMAGE ) && !(parent && BitTestWW(parent->winGetStyle(), GWS_SCROLL_LISTBOX)))
	...
	BitClearWW( statusFlags, WIN_STATUS_HIDDEN );
																								  ...
																								  Bool defaultVisual )
{
  ...
  ComboBoxData *comboBoxData;
	...
	// we MUST have a push button style window to do this
	if( BitTestWW( instData->getStyle(), GWS_COMBO_BOX) == FALSE )
	{
...
// begin here
  ...
  comboBox->winSetUserData( comboBoxData );
	...
	// Set up list box redraw callbacks
	if( BitTestWW( comboBox->winGetStatus(), WIN_STATUS_IMAGE ))
...

{
	...
	// we MUST have a push button style window to do this
	if( BitTestWW( instData->getStyle(), GWS_PROGRESS_BAR ) == FALSE )
	{
	...
	// on the device level of the engine
	//
	if( BitTestWW( progressBar->winGetStatus(), WIN_STATUS_IMAGE ) )
...

{
  ...
  TextData *data;
	...
	BitClearWW( instData->m_style, GWS_TAB_STOP );

  if( BitTestWW( instData->getStyle(), GWS_STATIC_TEXT ) ) 
	{
  ...
  if( textWin != NULL )
	{
		...
		if( BitTestWW( textWin->winGetStatus(), WIN_STATUS_IMAGE ) )
    ...
    memcpy( data, textData, sizeof(TextData) );
		...
		data->text->setWordWrapCentered( BitTestWW( instData->getStatus(), WIN_STATUS_WRAP_CENTERED ));
...

{
	...
	if( BitTestWW( instData->getStyle(), GWS_ENTRY_FIELD ) == FALSE )
	{
	...
	if( BitTestWW( entry->winGetStatus(), WIN_STATUS_IMAGE ) )
																								 ...
																								 Bool assignVisual )
{
	...
	if( BitTestWW( instData->getStyle(), GWS_PUSH_BUTTON ) )
	{
	...
	else if( BitTestWW( instData->getStyle(), GWS_CHECK_BOX ) )
	{
	...
	else if( BitTestWW( instData->getStyle(), GWS_RADIO_BUTTON ) )
	{
	...
	else if( BitTestWW( instData->getStyle(), GWS_HORZ_SLIDER ) )
	{
	...
	else if( BitTestWW( instData->getStyle(), GWS_SCROLL_LISTBOX ) )
	{
	...
	else if( BitTestWW( instData->getStyle(), GWS_COMBO_BOX ) )
	{
//		ComboBoxData *comboBoxData = (ComboBoxData *)gadget->winGetUserData();
	...
	else if( BitTestWW( instData->getStyle(), GWS_PROGRESS_BAR ) )
	{
	...
	else if( BitTestWW( instData->getStyle(), GWS_STATIC_TEXT ) )
	{
	...
	else if( BitTestWW( instData->getStyle(), GWS_ENTRY_FIELD ) )
	{
...
GameWindow *GameWindowManager::getWindowUnderCursor( Int x, Int y, Bool ignoreEnabled )
{
	...
	else
	{
		...
		for( window = m_windowList; window; window = window->m_next )
		{
			...
			{
				if( BitTestWW( window->m_status, WIN_STATUS_ENABLED ) || ignoreEnabled )
				{
					// determine which child window the mouse is in
	...
	if( window )
	{
		...
		else if( ignoreEnabled && !( BitTestWW( window->m_status, WIN_STATUS_ENABLED ) ))
		{
			window = NULL;
		}
	}
