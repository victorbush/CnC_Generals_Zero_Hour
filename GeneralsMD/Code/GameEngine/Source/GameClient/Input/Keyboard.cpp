...
void Keyboard::createStreamMessages( void )
{
	...
	while( key->key != KEY_NONE )
	{
		...
		if( BitTestWW( key->state, KEY_STATE_DOWN ) )
		{
		...
		else if( BitTestWW( key->state, KEY_STATE_UP ) )
		{
...
void Keyboard::updateKeys( void )
{
	...
	while( m_keys[ index ].key != KEY_NONE )
	{
		...
		{
			if( BitTestWW( m_keyStatus[ KEY_LALT ].state, KEY_STATE_DOWN ) ||
					BitTestWW( m_keyStatus[ KEY_RALT ].state, KEY_STATE_DOWN ) )
			{
...
//		return FALSE;
	...
	for( key = 0; key < 256; key++ )
	{
		...
		if( BitTestWW( m_keyStatus[ key ].state, KEY_STATE_DOWN ) )
		{
