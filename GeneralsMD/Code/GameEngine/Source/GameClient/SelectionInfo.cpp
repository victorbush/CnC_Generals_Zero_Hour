...
UnsignedInt getPickTypesForContext( Bool forceAttackMode )
{
	...
	if (command != NULL) {
		if (BitTestWW( command->getOptions(), ALLOW_MINE_TARGET)) {
			types |= PICK_TYPE_MINES;
		}

		if (BitTestWW( command->getOptions(), ALLOW_SHRUBBERY_TARGET ) ) {
...
//		}
		...
		// For efficiency.
		if (BitTestWW(retVal, PICK_TYPE_MINES | PICK_TYPE_SHRUBBERY)) {
			break;
		}
	}
...
{
	if (BitTestWW(pickTypes, PICK_TYPE_SELECTABLE)) {
		outMask.set(KINDOF_SELECTABLE);
	}

	if (BitTestWW(pickTypes, PICK_TYPE_SHRUBBERY)) {
		outMask.set(KINDOF_SHRUBBERY);
	}

	if (BitTestWW(pickTypes, PICK_TYPE_MINES)) {
		outMask.set(KINDOF_MINE);
	}

	if (BitTestWW(pickTypes, PICK_TYPE_FORCEATTACKABLE)) {
