...
void nickErrorCallbackWrapper( PEER peer, Int type, const char *nick, int numSuggestedNicks, const char** suggestedNicks, void *param )
{
	if (param != NULL)
	{
														...
														int numPlaying, void * param)
{
	...
	if (success)
	{
		...
		{
			resp.groupRoomName.clear(); // .empty();
		}
		TheGameSpyPeerMessageQueue->addResponse(resp);
#ifdef SERVER_DEBUGGING
		CheckServers(peer);
...
void gameStartedCallback( PEER peer, SBServer server, const gsi_char* message, void *param )
{
	PeerResponse resp;
