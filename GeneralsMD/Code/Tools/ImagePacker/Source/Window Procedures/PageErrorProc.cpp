		...
		case WM_INITDIALOG:
		{
					 ...
					 page = page->m_next )
			{
				...
				if( BitTestWW( page->m_status, TexturePage::PAGE_ERROR ) )
				{
					...
					if( BitTestWW( page->m_status, TexturePage::CANT_ALLOCATE_PACKED_IMAGE ) )
						sprintf( reason, "Can't allocate image memory" );
					else if( BitTestWW( page->m_status, TexturePage::CANT_ADD_IMAGE_DATA ) )
						sprintf( reason, "Can't add image(s) data" );
					else if( BitTestWW( page->m_status, TexturePage::NO_TEXTURE_DATA ) )
						sprintf( reason, "No texture data to write" );
					else if( BitTestWW( page->m_status, TexturePage::ERROR_DURING_SAVE ) )
